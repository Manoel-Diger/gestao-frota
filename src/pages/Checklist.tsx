...
import { Plus, Eye, Trash2, FileCheck, AlertTriangle, CheckCircle, Search } from "lucide-react";
...
export default function ChecklistPage() {
    ...
    const getMotoristaNome = (id: number | string) => {
        ...
        return motorista ? motorista.nome || '' : String(id);
    };
    ...
    const filteredChecklists = useMemo(() => {
        ...
        return checklists.filter(checklist => {
            const motoristaNome = getMotoristaNome(checklist.motorista).toLowerCase();
            const placaVeiculo = checklist.placa_veiculo.toLowerCase();
            const tipoChecklist = checklist.tipo_checklist.toLowerCase();

            return motoristaNome.includes(lowerCaseSearch) ||
                       placaVeiculo.includes(lowerCaseSearch) ||
                       tipoChecklist.includes(lowerCaseSearch);
        });
    }, [checklists, searchTerm, motoristas]);
                ...
                <CardHeader>
                    <div className="flex flex-col sm:flex-row gap-4">
                        <div className="relative flex-1">
                            ...
                            <Input
                                placeholder="Buscar por motorista, placa ou tipo..."
                                value={searchTerm}
                ...
                <CardContent>
                    {loading ? (
                        ...
                        <div className="text-center py-8 text-muted-foreground">
                            ...
                            <p>{searchTerm ? "Nenhum checklist encontrado para sua pesquisa." : "Nenhum checklist encontrado."}</p>
                            {!searchTerm && <p className="text-sm">Clique em "Novo Checklist" para começar.</p>}
                        </div>
                    ) : (
            ...
            <Dialog open={showForm} onOpenChange={setShowForm}>
                <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Preencha os campos abaixo para cadastrar um novo checklist.
                        </DialogDescription>
                    </DialogHeader>
                    <ChecklistForm
                        onSuccess={() => {
            ...
            <Dialog open={showDetails} onOpenChange={setShowDetails}>
                <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Visualize todas as informações preenchidas no checklist selecionado.
                        </DialogDescription>
                    </DialogHeader>
