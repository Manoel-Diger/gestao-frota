...
export function useChecklists() {
  // Inicializa o estado com dados do localStorage, se existir, ou vazio
  ...
  const createChecklist = useCallback(async (checklistData: ChecklistInsert) => {
    ...
    try {
      ...
      // Atualiza estado e persiste no localStorage
  ...
  const updateChecklist = useCallback(async (id: string, updateData: ChecklistUpdate) => {
    ...
    try {
      ...
      persistChecklists(checklists.map(c => (c.id === id ? updatedChecklist : c)));
